...
def save_authors(data):
    for dt in data:
        author = Author(fullname=dt['fullname'],born_date=datetime.strptime(dt['born_date'],"%B %d, %Y"),born_location=dt['born_location'],description=dt['description'])
        author.save()
...
def save_quotes(quotes_data):
    for dt in quotes_data:
        quote = Quote(tags = dt['tags'],author = find_author_id(dt['author']),quote=dt['quote'])
        quote.save()
    return 'Quotes Saved!'
